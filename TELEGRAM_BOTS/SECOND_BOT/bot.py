...
#В JSON не надо вписывать "," (СТРОКА 89 , РАНЬШЕ ПОСЛЕ ЮЗЕРА СТОЯЛА ЗАПЯТАЯ) ,так как он обычно ведет запись через запятую сам (или это телега так делает , лол)
...
def echo_digits(message:Message):
    ...
    if message.from_user.username in info:
        ...
        with open (fil, "w") as f:
            data=str(info) , message.from_user.username   # ТУТ ЗАПЯТАЯ НАХУЙ НЕ НУЖНА
            json.dump(data,f)
